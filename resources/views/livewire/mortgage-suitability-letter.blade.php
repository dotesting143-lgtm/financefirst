<x-slot name="header">
        <h2 class="font-bold text-lg xl:text-2xl text-white">
            {{ __('Mortgage Suitability Letter') }}
        </h2>
</x-slot>
                ...
                <div class="suitability-form bg-white sm:px-4 lg:p-6">
                    <div class="grid grid-cols-1 gap-4 lg:gap-6">
                        <div class="col-span-full">
                            <label for="mortsuit_letter" class="block oxford-blue text-xs lg:text-sm font-medium mb-1 lg:mb-2">{{ __('Mortgage Suitability Letter Text') }}<span class="text-red-500 ml-0.5">*</span></label>
                            <textarea id="mortsuit_letter" wire:model.defer="mortsuit_letter" class="block text-xs lg:text-sm font-medium appearance-none border border-pale-blue-gray rounded-md w-full p-2.5 lg:p-3 text-black focus:outline-none focus:shadow-outline focus:border-oxford-blue focus:ring-oxford-blue placeholder:text-cool-gray" required rows="12"></textarea>
                            @error('mortsuit_letter') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                        </div>
                    </div>
